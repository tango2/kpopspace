...
with open(output_file, "wb") as output:
    ...
    with open("blackpink_figures.json") as f:
        lines = f.readlines()
        count = 0
        frame_group = 0
        previous_frame =""
        ...
        for idx, val in enumerate(lines):
            frame_number_value = ""
            delimited_line = val.split("parts")
        ...
        # print len(delimited_line)
            bodies = len(delimited_line) # get the parts
            part_check = re.split(r'"[0-9]+":',val)            
            # count the parts
            number_parts = len(part_check)   
            # if there is 1 body and more than 7 parts...
            if bodies == 2 and number_parts > 7:
                # add the data                      
